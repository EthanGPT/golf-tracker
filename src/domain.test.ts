...
describe('distance calculations', () => {
  ...
  it('calculates the median and excludes mishits', () => {
    ...
    const readings = [...data.readings, { id: 'mishit', club: '6i' as const, distanceMetres: 200, mishit: true, sessionDate: '2026-09-14', createdAt: '2026-09-15', severeMiss: true }]
...
describe('round feedback', () => {
  const rounds = [{ id: 'r1', date: '2026-09-15', courseName: 'Hermanus Golf Club', totalScore: 45, overallNote: '', status: 'archived' as const, holes: [{ holeNumber: 1, score: 5, focusCategory: 'Approach' as const, wentRight: 'Good tempo', wentWrong: 'Missed green', tags: [{ category: 'iron' as const, outcome: 'Short', type: 'went-wrong' as const }] }] }]
  it('totals scores', () => expect(roundTotal(rounds[0])).toBe(5))
  it('counts problem categories and explains recommendations', () => { expect(categoryCounts(rounds).Approach).toBe(1); expect(recommendation(rounds).text).toContain('Not enough repeated feedback') })
})
