...
function Onboarding({
  ...
  const onboardingTees = getCourse(onboardingCourseId)?.tees;
  const teeOptions = onboardingTees?.map((tee) => tee.id) || ["yellow", "white", "blue", "red"];
  ...
  return <div className="onboarding-screen">
    ...
    {step === 1 && <div className="onboarding-step"><span className="eyebrow">YOUR PREFERENCE</span><h1>Which tees do you<br /><em>usually play?</em></h1><p>We’ll use this for round setup and hole strategy.</p><div className="tee-choices">{teeOptions.map((tee) => { const definition = onboardingTees?.find((item) => item.id === tee); const label = definition?.name || `${tee[0].toUpperCase()}${tee.slice(1)}`; return <button key={tee} className={preferredTee === tee ? "selected" : ""} onClick={() => setPreferredTee(tee)}><span className={`tee-dot ${definition?.colour || tee}`} />{label}{preferredTee === tee && <Check size={17} />}</button>; })}</div></div>}
